...
use signal_kt_auditor::{TransparencyLog, auditor::{Auditor, PublicConfig, DeploymentMode}};
use signal_kt_auditor::test_vectors;
...
fn benchmark_sequential_log_updates(c: &mut Criterion) {
    ...
    let should_succeed = test_vectors.should_succeed.expect("No should_succeed test vectors found");
    ...
    group.bench_function("sequential_updates", |b| {
        ...
        b.iter(|| {
            ...
            log.apply_update(update.update.as_ref().unwrap().clone()).unwrap();
...
fn benchmark_head_signing(c: &mut Criterion) {
    ...
    group.bench_function("sign_head", |b| {
        b.iter(|| {
            let signature = auditor.sign_head(black_box(final_root), black_box(final_size)).unwrap();
            black_box(signature);
        });
    });
